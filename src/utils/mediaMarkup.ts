...
export function renderImageMarkup(
    ...
    const loadHint = masked ? '<div class="image-load-hint">显示图片</div>' : '';

    return `<div class="${classes}" style="${imageStyle}" data-iw="${metaWidth}" data-ih="${metaHeight}"${dataSrc}><img src="${displaySrc}" data-full-src="${safeSrc}" class="${imageClasses}" alt="image" loading="lazy" decoding="async" referrerpolicy="no-referrer">${loadHint}</div>`;
}

export function renderAudioMarkup(src: string) {
    const safeSrc = escapeHTML(src);
    return `<div class="audio-player-container"><audio controls preload="metadata"><source src="${safeSrc}">您的浏览器不支持音频播放。</audio></div>`;
}

export function renderVideoMarkup(src: string) {
    const safeSrc = escapeHTML(src);
    return `<div class="video-player-container"><video controls preload="metadata"><source src="${safeSrc}" type="video/mp4"><source src="${safeSrc}" type="video/webm">您的浏览器不支持视频播放。</video></div>`;
}
